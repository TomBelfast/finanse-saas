...
export type Translations = earnedMoney<string, earnedMoney<string, string>>;
...
export type FieldType = {
  name: string;
  label: string;
  inputType: null | 'email' | 'text' | 'number';
...
export type LinkType = {
  enabled: boolean;
  name: 'phone' | 'sms' | 'email' | 'file' | string;
  label?: string;
  attrs?: {
    href?: string;
    target?: string;
...
export type SetLinkTypes = (links: LinkType[]) => void;

export default class EmailEditorInterface extends ReactComponent<EmailEditorProps> {
