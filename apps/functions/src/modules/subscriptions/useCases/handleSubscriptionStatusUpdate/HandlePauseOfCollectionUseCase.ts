...
export class HandlePauseOfCollection
  implements StripeUseCase<HandleSubscriptionStatusUpdateDTO, Promise<Response>>
{
  ...
  async execute({ subscription }: HandleSubscriptionStatusUpdateDTO): Promise<Response> {
    const { logger, paymentClient } = this.dependencies;

    logger.info('Start updating pause of payment collection');
    ...
    logger.info('Successfully update info about paused collection for subscription', { item });
