...
async function checkUser(email) {
    ...
    try {
        connection = await mysql.createConnection({
            host: '192.168.0.9',
            user: 'Saas', // Zaktualizowany user
            password: process.env.DB_PASSWORD || 'Finanse2025', // Zaktualizowane hasło
            database: 'Finanse', // Zaktualizowana nazwa bazy
            port: 3306
        });
        ...
        const [rows] = await connection.execute(
            'SELECT uid, email, contact_email, created_at FROM users WHERE email LIKE ? OR contact_email LIKE ?',
            [`%${email}%`, `%${email}%`]
        );
